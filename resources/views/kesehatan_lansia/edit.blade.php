                ...
                <div class="card-body">
                    ...
                    <h2 class="card-title fw-semibold mb-4">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            Edit Data Cek Kesehatan Lansia <span class="badge rounded-pill text-bg-info">{{ $lansia->nama }}</span>
                        </div>
                    </h2>
                    <div class="mt-4">
                        <form class="" method="post" action="{{ route('kesehatan_lansia.update', $kesehatan->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="modal-body">
                                <div class="row gap-0 row-gap-3 mb-3">
                                    ...
                                    <div class="col-sm-12 col-md-6 col-lg-3">
                                        ...
                                        <input id="bb" class="form-control form-sm" name="bb" type="number" value="{{ $kesehatan->bb }}" step="0.1" required>
                                    </div>
                                    <div class="col-sm-12 col-md-6 col-lg-3">
                                        <label class="form-label" for="tb">Tinggi Badan (Cm)</label>
                                        <input id="tb" class="form-control form-sm" name="tb" type="number" value="{{ $kesehatan->tb }}" step="0.1" required>
                                    </div>
                                    <div class="col-sm-12 col-md-6 col-lg-3">
                                        <label class="form-label" for="tekanan_darah">Tekanan Darah (mmHg)</label>
                                        <input id="tekanan_darah" class="form-control form-sm" name="tekanan_darah" type="text" value="{{ $kesehatan->tekanan_darah }}" step="0.1" required>
                                    </div>
                                    <div class="col-sm-12 col-md-6 col-lg-3">
                                        <label class="form-label" for="kolestrol">Kolestrol (mg/dL)</label>
                                        <input id="kolestrol" class="form-control form-sm" name="kolestrol" type="text" value="{{ $kesehatan->kolestrol }}" required>
                                    </div>
                                    <div class="col-sm-12 col-md-6 col-lg-3">
                                        <label class="form-label" for="gula_darah">Gula Darah (mg/dL)</label>
                                        <input id="gula_darah" class="form-control form-sm" name="gula_darah" type="text" value="{{ $kesehatan->gula_darah }}" required>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <h6><b>Tambahan</b></h6>
                                    <div class="col-sm-12 col-md-6 col-lg-4">
                                        ...
                                        <textarea id="keluhan" name="keluhan" class="form-control">{{ $kesehatan->keluhan }}</textarea>
                                    </div>
                                    <div class="col-sm-12 col-md-6 col-lg-4">
                                        <label class="form-label" for="catatan">Catatan</label>
                                        <textarea id="catatan" name="catatan" class="form-control">{{ $kesehatan->catatan }}</textarea>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <h6><b>Petugas</b></h6>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response, function(index, data) {
                    let selected = '';
                    if (data.id == '{{ $kesehatan->petugas_id }}') {
                        selected = 'selected';
                    }
